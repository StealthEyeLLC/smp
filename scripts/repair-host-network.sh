...
ensure_rule() {
    ...
    if [[ $table == filter ]]; then
        iptables -C "$@" >/dev/null 2>&1 || iptables -I "$@"
    else
        iptables -t "$table" -C "$@" >/dev/null 2>&1 || iptables -t "$table" -I "$@"
    fi
}

# These rules are inserted into the host's canonical iptables-nft chains so
# they precede a default FORWARD drop from UFW or the distribution firewall.
ensure_rule filter FORWARD 1 -i "$TAP" -j ACCEPT
ensure_rule filter FORWARD 1 -o "$TAP" -m conntrack --ctstate ESTABLISHED,RELATED -j ACCEPT
ensure_rule nat POSTROUTING 1 -s "$SUBNET" -o "$OUTBOUND" -j MASQUERADE

# Locally generated traffic does not traverse PREROUTING. Mirror every SMP
# published TCP/UDP port into nat OUTPUT so localhost acceptance is real.
while IFS=$'\t' read -r protocol host_port guest_port; do
    [[ -n $protocol ]] || continue
    ensure_rule nat OUTPUT 1 -p "$protocol" -d 127.0.0.1 --dport "$host_port" -j DNAT --to-destination "$GUEST:$guest_port"

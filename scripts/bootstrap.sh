...
if [[ $SKIP_PACKAGES -eq 0 ]]; then
    ...
    apt-get install -y --no-install-recommends \
      ...
      xz-utils zstd bison flex libelf-dev bc dwarves rsync file kmod procps
fi

for tool in cargo rustc curl git install systemctl sha256sum; do
...
install -m 0755 "$SOURCE/scripts/build-assets.sh" /usr/lib/smp/build-assets.sh
install -m 0755 "$SOURCE/scripts/create-seed.sh" /usr/lib/smp/create-seed.sh
...
cat > /etc/smp/install.json.new <<INSTALL
{
  ...
  "commit": "$COMMIT",
  "smpVersion": "$(/usr/local/bin/smp version --json 2>/dev/null | jq -r .version)",
...
/usr/local/bin/smp describe >/dev/null
